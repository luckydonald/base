...
- Claude Code has a real, git-trackable settings key `includeCoAuthoredBy` (in `.claude/settings.json`, which is committed — unlike `.claude/settings.local.json`) that suppresses the tool's own default "🤖 Generated with Claude Code" / `Co-Authored-By:` footer. It's not currently set anywhere in `base`. Setting it to `false` is the "disable via git-tracked per-repo config" the user asked for, via the shared/tracked key `allowCoAuthoredEtc` in `ai/tool-settings/settings.json` (named distinctly from the native Claude Code key since it's our own toggle, rendered into it by `render_claude`). Note: it only suppresses Claude Code's *built-in* footer — the custom `Claude-Session:` line comes from session-level attribution instructions, not from that setting, so the pre-commit hook remains the actual enforcement backstop for both trailers.
...
2. **Add repo-level config to suppress Claude Code's own footer**: set `"allowCoAuthoredEtc": false` in the git-tracked `ai/tool-settings/settings.json` (source of truth) and let `sync.py` render it into `.claude/settings.json`'s native `includeCoAuthoredBy` key; extend `render_claude` in `°settings_lib/hooks.py` to pass the key through.
3. **Add project memory**: write `ai/°base/memory/repo_commit_hooks.md` stating both `Co-Authored-By:` and `Claude-Session:` are rejected by the hook, and that `allowCoAuthoredEtc: false` is set (rendering into `includeCoAuthoredBy: false`). Add its index line to `ai/°base/memory/MEMORY.md`.
